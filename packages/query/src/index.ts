...
export interface QueryPredicate { field: string; value: string; }
export interface QueryExpression { predicates: QueryPredicate[]; }
export interface QueryResult { expression: QueryExpression; objects: CodexObject[]; count: number; }
...
export function executeQuery(graph: CodexGraph, query: string | QueryExpression): QueryResult {
  ...
  const objects = [...graph.objects.values()].filter((object) => expression.predicates.every((predicate) => matches(object, predicate)));
