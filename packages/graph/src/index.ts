...
export class CodexGraph {
  ...
  neighbours(id: string): readonly CodexObject[] {
    const ids = new Set([...this.edgesFrom(id).map((edge) => edge.target), ...this.edgesTo(id).map((edge) => edge.source)]);
  ...
  statistics(): GraphStatistics {
    ...
    return { objects: this.objects.size, relations: [...this.outgoing.values()].reduce((count, edges) => count + edges.length, 0), roots: this.roots().length, maxDepth, objectTypes, languages };
  }
