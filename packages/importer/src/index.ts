...
export async function compileProject(root: string, options: CompileOptions = {}): Promise<CompileResult> {
  const projectDescriptor = await readProjectDescriptor(root);
  ...
  const project: CodexProject = {
    codexVersion: options.codexVersion ?? stringValue(projectDescriptor.codexVersion) ?? "0.2.0",
    id: stringValue(projectDescriptor.id) ?? path.basename(path.resolve(root)),
    title: stringValue(projectDescriptor.title) ?? path.basename(path.resolve(root)),
    profile: options.profile ?? stringValue(projectDescriptor.profile),
    objects
  };
...
function toObject(document: ParsedDocument): CodexObject {
  ...
  const title = stringValue(document.metadata.title) ?? firstHeading(document.body) ?? id;
  const derivedFrom = stringArray(document.metadata.derivedFrom);
