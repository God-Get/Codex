...
test("HERMETICA reference corpus compiles into runtime graph", async () => {
  ...
  assert.equal(result.graph.getObject("WORK-CH")?.type, "work");
  assert.deepEqual(result.graph.descendants("WORK-CH", ["contains"]).map((object) => object.id), ["FRAG-CH-01"]);
  assert.equal(result.graph.edgesTo("FRAG-CH-01").length, 2);
});
...
test("HERMETICA reference corpus supports translation queries", async () => {
  ...
  assert.equal(result.objects[0]?.id, "TRANS-RU-CH-01");
});
